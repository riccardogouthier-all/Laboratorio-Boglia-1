...
canzoniRouter.post('/', (req, res) => {
  ...
  db.run(INSERT_CANZONE, [titolo, artista, album ?? null, genere ?? null, anno ?? null, durata_secondi ?? null], function (err) {
    if (err) return res.status(500).json({ error: err.message });
    const nuovaCanzone = { id: this.lastID, titolo, artista, album, genere, anno, durata_secondi };
...
canzoniRouter.put('/:id', (req, res) => {
  ...
  const { titolo, artista, album, genere, anno, durata_secondi } = req.body;
  db.run(UPDATE_CANZONE, [titolo, artista, album ?? null, genere ?? null, anno ?? null, durata_secondi ?? null, req.params.id], function (err) {
    if (err)           return res.status(500).json({ error: err.message });
    if (!this.changes) return res.status(404).json({ error: 'Canzone non trovata' });
    publishEvent('song.updated', { id: Number(req.params.id), titolo, artista, album, genere, anno, durata_secondi });
...
canzoniRouter.patch('/:id', (req, res) => {
  ...
  db.get(SELECT_BY_ID, [req.params.id], (err, canzone) => {
    ...
    db.run(UPDATE_CANZONE, [updated.titolo, updated.artista, updated.album, updated.genere, updated.anno, updated.durata_secondi, req.params.id], function (err2) {
